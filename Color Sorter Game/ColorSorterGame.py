...
new_game = True
#random level could have 10-14 tubes, always two empty
tubes = 10
tube_colors = []
color_choices = ['red', 'orange','light blue','cyan','light green',
                 'dark green', 'purple', 'dark blue', 'pink', 'brown', 'yellow','dark gray']
selected = False
select_rect = 100
dest_rect = 100
win = False
...
#tube drawing on the screen with the colores rectangles inside
    ...
    if offset:
        for i in range(tubes_per_row - 1):
            for j in range(len(tube_cols[i+tubes_per_row])):
                pygame.draw.rect(screen, color_choices[tube_cols[i+tubes_per_row][j]], [(spacing*0.5) + 5 +
                                spacing * i, 450 - (50 * j), 65, 50], 0,3)
            box = pygame.draw.rect(screen, 'white', [(spacing*0.5) + 5 + spacing * i, 300, 65, 200],5, 5)
        ...
        for i in range(tubes_per_row):
            for j in range(len(tube_cols[i + tubes_per_row])):
                pygame.draw.rect(screen, color_choices[tube_cols[i + tubes_per_row][j]], [5 +
                            spacing * i,450 - (50 * j), 65, 50],0, 3)
...
#determine the top color of the selected tube and the destination tube
...
#check to see if the vid=c=tory conditions are met
...
while(run):
    screen.fill('black')
    timer.tick(fps)
    #generate a new board/tubes on new game conditions
    if new_game:
...
#draw victory text if we have won,
#and leave instructions on how to restart and get new board
    if win:
        victory_text = font.render('You win! Press Enter for a new board!', True, 'white')
        screen.blit(victory_text,(30,265))
    restart_test = font.render('Stuck? Space-Restart, Enter-New Board!', True, 'white')
    screen.blit(restart_test, (10,10))

#display.flip draws items into the screen, quit exits the program
    pygame.display.flip()
pygame.quit()